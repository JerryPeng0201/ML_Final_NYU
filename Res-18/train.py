...
from model import  ResNet,Regressor,weights_normal_init
...
parser.add_argument("-dp", "--data_path", type=str, default='/scratch/jp4906/FSC147_384_V2/', help="Path to the FSC147 dataset")
parser.add_argument("-o", "--output_dir", type=str,default='./runs/train2', help="/Path/to/output/logs/")
...
train_meta_file = data_path + 'train.json'
test_meta_file = data_path + 'test.json'
val_meta_file = data_path + 'val.json'
...
for epoch in range(0,args.epochs):
    ...
    if best_mae >= val_mae:
        best_mae = val_mae
        best_rmse = val_rmse
        model_name = args.output_dir + '/' + "FamNet_best_" + str(epoch) + '.pth'
        torch.save(regressor.state_dict(), model_name)
    if epoch % 10 == 0:
        model_name = args.output_dir + '/' + "FamNet_" + str(epoch) + '.pth'
        torch.save(regressor.state_dict(), model_name)
